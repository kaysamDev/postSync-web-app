				...
				<nav className="mt-4">
					{links.map((link, index) => (
						<div key={index}>
							<Link
								to={link.href}
								className={`flex items-center py-2.5 px-4 transition duration-200 hover:bg-white/9 ${
									location.pathname === link.href ? "bg-white/20" : ""
								}`}
								...
								<div className={`ml-8 ${isOpen ? "block" : "hidden"}`}>
									{link.subLinks.map((subLink, subIndex) => (
										<Link
											...
											to={subLink.href}
											className={`flex items-center py-2.5 px-4 transition duration-200 hover:bg-white/9 ${
												location.pathname === subLink.href ? "bg-white/20" : ""
											}`}
										>
